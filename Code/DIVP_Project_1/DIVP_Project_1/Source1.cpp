// EEE 598 : DIVP Project 1: Stereo Correspondence using FAST+FREAK, AGAST+LUCID and Sift
...
int   main(int argc, char** argv)
{
	...
	cout << " Output For Without Cross Check and Without Threshold " << endl;
	...
	//DMatch f; float mini_fast = 200;
	//for (int k = 0; k < matches_fast_freak.size(); k++)
	//{
		//f = matches_fast_freak[k];
		//if (f.distance < 3 * mini_fast)
			//matches_Fast_Freak.push_back(matches_fast_freak[k]);
	//}
	matches_Fast_Freak = matches_fast_freak;  // Remove this line for thresholding
	...
	imwrite("D:\\ASU_Sem_1\\DIVP_Project\\Output_images\\img_matches_fast_wo_cross_wo_thresh.png", img_matches_fast);
	...
	std::vector< DMatch > matches_Sift;
	//DMatch A; float mini = 30;
	matcher.match(desc1_Sift, desc2_Sift, matches_sift);
	// -------- For Thresholding ------

	//for (int k = 0; k < matches_sift.size(); k++)
	//{
		//A = matches_sift[k];
		//if (A.distance < 3 * mini)
		//matches_Sift.push_back(matches_sift[k]);
	//}
	matches_Sift = matches_sift; // Remove this line for thresholding
	...
	imwrite("D:\\ASU_Sem_1\\DIVP_Project\\Output_images\\img_matches_Sift_wo_cross_wo_thresh.png", img_matches_Sift);
	...
	std::vector< DMatch > matches_agast_lucid;
	DMatch a; float mini_lucid = 10;
	matcher.match(desc1_Lucid, desc2_Lucid, matches_agast_lucid);

	// -------- For Thresholding ------
	//for (int k = 0; k < matches_sift.size(); k++)  
	//{
		//a = matches_agast_lucid[k];
		//if (a.distance < 3 * mini_lucid)
		//matches_Agast_Lucid.push_back(matches_agast_lucid[k]);
	//}
	matches_Agast_Lucid = matches_agast_lucid; // Remove this line for thresholding
	...
	imwrite("D:\\ASU_Sem_1\\DIVP_Project\\Output_images\\img_matches_Agast_Lucid_wo_cross_wo_thresh.png", img_matches_Agast_Lucid);
